...
def get_template_by_key(
    ...
    if current_user.role != "admin" and template.created_by not in (None, current_user.id):
...
def update_template(
    ...
    write_audit_log(
        ...
        detail={"before": before, "after": {"name": template.name, "version": template.version, "key": template.key}},
    )
    db.commit()
